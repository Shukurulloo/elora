...
app.use(
  // middleware pattern asosida integratsya
  cors({
    // cors ihtiyoriy domendan kelayotgan requestlarni  servermga kirishiga rusat beradi
...
app.use(
  session({
    // yuqoridagi sessionni qo'yib uni ichiga obtionlarni beramiz
...
app.use(function (req, res, next) {
  // hamma qilinyotgan requestlar uchn ishga tushadi
...
const server = http.createServer(app); // severni toliq qolga olamz
...
let summeryClient = 0; // serverga ulangalar soni
io.on("connection", (socket) => { // connection bolsa (socket) buyerda qabul qilamz
  summeryClient++; // ulansa birga oshir
  console.log(`Connection & total [${summeryClient}]`); //info

  socket.on("disconnect", () => {
    summeryClient--; // uzilsa kamaytir
  console.log(`Disconnection & total [${summeryClient}]`); //info

  })
});

export default server; // endi appni emas serverni yuboramz
